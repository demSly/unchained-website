...
const Address = ({
  ...
  (
    <address>
      ...
      {addressLine}<br />
      {postalCode} {city} {countryCode && `(${countryCode})`}<br />
      {countryName}
    </address>
  );
  ...
  (
    <div>
      <div className="flex-between">
        <div>
          <small className="address-label">{intl.formatMessage({ id: 'email' })}</small>
          <span>{emailAddress}</span>
        </div>
        <button className="button button--small" onClick={onEdit}>{intl.formatMessage({ id: 'edit' })}</button>
      </div>
      {invoiceAddress && (
        <div>
          <small className="address-label">{!deliveryAddress ? intl.formatMessage({ id: 'address' }) : intl.formatMessage({ id: 'billing_address' })}</small>
          <Address {...invoiceAddress} countryName={countryName} />
        </div>
      )}
      {deliveryAddress && (
        <div>
          <small className="address-label">{intl.formatMessage({ id: 'delivery_address' })}:</small>
